...
public:
  ...
  /// Write one byte to socket \p handle for each new response, without taking the GIL.
  /**
   * \p handle must be a non-blocking socket, and stays owned by the caller: call
   * clear_on_new_response_callback() before closing it. Replaces any on new response callback.
   */
  void
